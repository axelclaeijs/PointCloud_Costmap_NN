# BAP 2019
# Author: Axel Claeijs
# This basic feed forward network will use cross-validation to train
...
GRAPH_EVAL_MAE = DIR + 'v' + str(VERSION) + '/evaluate_mae' + str(VERSION) + '.png'
...
for i in range(k):
    ...
    val_label = label_train[i * num_val_samples: (i + 1) * num_val_samples]
    ...
    partial_label_train = np.concatenate(
    ...
    model = tf.keras.Sequential([
        ...
        tf.keras.layers.Dense(1200, activation=tf.nn.relu, kernel_initializer='random_uniform',
                              bias_initializer='zeros'),
        tf.keras.layers.Dense(1600, activation=tf.nn.sigmoid, kernel_initializer='random_uniform',
                              bias_initializer='zeros')
    ])
...
# The average of the per-epoch MAE scores
plt.figure()
plt.title('Validation MAE')
plt.xlabel('Epoch')
plt.ylabel('Mean Absolute Error [$MPG^2$]]')
plt.plot(range(1, len(average_mae_history)+1), average_mae_history)
plt.savefig(GRAPH_EVAL_MAE)
...
plt.figure()
plt.title('Evaluation model loss')
plt.xlabel('Epoch')
plt.ylabel('Loss')
